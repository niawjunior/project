...
<html>
  ...
  <body>
    ...
    {
    $objQuery = mysqli_query($connect,"INSERT INTO data (h1,h2,la,lo,deep,url) VALUES ('".$_POST["txtAddh1"]."','".$_POST["txtAddh2"]."','".$_POST["txtAddla"]."','".$_POST["txtAddlo"]."','".$_POST["txtAdddeep"]."','logo.png')");
    }
    else
    {
    ...
    <form name="frmMain" method="post" action="<?=$_SERVER["PHP_SELF"];?>" enctype="multipart/form-data">
      <table class="table table-hover" border="0" id="">
        <tr>
        ...
        <tr>
          <td><center>
            <input class="form-control" style="text-align:center;" name="txth1" type="text" value="<?=$f1?>">
          </center></td>
          <td><center>
            <input class="form-control" style="text-align:center;" name="txth2" type="text" value="<?=$f2?>">
          </center></td>
          <td><center>
            <input class="form-control" style="text-align:center;" name="txtla" type="text" value="<?=$f3?>">
          </center></td>
          <td><center>
            <input class="form-control" style="text-align:center;" name="txtlo" type="text" value="<?=$f4?>">
          </center></td>
          <td><center>
            <input class="form-control" style="text-align:center;" name="txtdeep" type="text" value="<?=$f5?>">
          </center></td>
        ...
        ?>
        <tr>
          <td><center><a class="text-primary" href="water_level.php?PAGE=<?=$f1?>" target="_blank"><?php echo $f1 ?></a></center></td>
          <td><center><?php echo substr($f2,0,105)?></center></td>
          ...
          ?>
          <td align="center"><a href="JavaScript:if(confirm('<?if($f1==$f111){$TT='ต้องการยกเลิกการใช้แผนที่หรือไม่?';}else{$TT='ต้องการใช้เป็นแผนที่หลักหรือไม่?';}?><?=$TT?>')==true){window.location='<?=$_SERVER["PHP_SELF"];?>?Page=<?=$Page?>&Action=<?if($f1==$f111){$UU='UnUse';$II=$f222;}else{$UU='Use';$II=$f0;}?><?=$UU?>&ID=<?=$II?>';}"><?=$ICON?></a></td>
          <td align="center"> <?=$icon?></td>
          <td align="center"><a href="JavaScript:if(confirm('ต้องการจะแก้ไขหรือไม่?')==true){window.location='<?=$_SERVER["PHP_SELF"];?>?Page=<?=$Page?>&Action=Edit&ID=<?=$objResult["ID"];?>';}"> <span class="glyphicon glyphicon-edit"></span></a></td>
          <td align="center"><a href="JavaScript:if(confirm('ต้องการจะลบหรือไม่?')==true){window.location='<?=$_SERVER["PHP_SELF"];?>?Action=Del&ID=<?=$f0?>';}"> <span class="glyphicon glyphicon-trash"></span></a></td>
        </tr>
        <?
        }
        ?>
